...
def generate_intelligent_hybrid_coordinates(hybrid_atoms, coordsA, coordsB_aligned, mapping, state, max_distance=1.0,
                                            min_dummy_distance=0.3):
    """
    ...
    # Step 2: Position dummy atoms intelligently
    placed_dummy_coords = []
...
def place_dummy_near_non_dummy(non_dummy_coord, existing_dummy_coords, max_distance, min_dummy_distance):
    """
    Place a dummy atom near a non-dummy atom while avoiding overlap with other dummies.

    Args:
        non_dummy_coord: Coordinates of the nearest non-dummy atom
        existing_dummy_coords: List of already placed dummy coordinates
        max_distance: Maximum distance from non-dummy atom
        min_dummy_distance: Minimum distance between dummy atoms

    Returns:
        Coordinates for the new dummy atom
    """
    import math
    import random
    ...
    for attempt in range(max_attempts):
        ...
        y = non_dummy_coord[1] + distance * math.sin(phi) * math.sin(theta)
        z = non_dummy_coord[2] + distance * math.cos(phi)

        candidate_coord = (x, y, z)

        # Check if this position is far enough from existing dummies
        too_close = False
        for existing_coord in existing_dummy_coords:
            dist = calculate_distance(candidate_coord, existing_coord)
            if dist < min_dummy_distance:
                too_close = True
                break
